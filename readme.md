...
### 1. **E-commerce: Procesamiento de Órdenes**
```
Usuario crea orden → eventos (orden_creada: P)
                   ↓ Kafka
                   ↓ Consumer procesa:
...
### 2. **Fintech: Transacciones Bancarias**
```
Transferencia → eventos (transferencia: P)
              ↓ Kafka (garantiza no se pierda)
              ↓ Consumer:
              ├─ Valida saldo
...
### 3. **Streaming: Netflix/Spotify**
```
Usuario reproduce video → eventos (reproduccion_iniciada: P)
                        ↓ Kafka
                        ↓ Consumers:
                        ├─ Analytics (qué ven los usuarios)
                        ├─ Recomendaciones (ML)
                        ├─ Billing (tiempo de uso)
                        └─ CDN (optimizar carga)
```

### 4. **Delivery: Uber Eats/Rappi**
```
Pedido creado → eventos (pedido_nuevo: P)
              ↓ Kafka
              ↓ Consumers:
...
### 5. **Redes Sociales: Instagram/Twitter**
```
Usuario publica post → eventos (post_creado: P)
                     ↓ Kafka
                     ↓ Consumers:
                     ├─ Genera feed de seguidores
                     ├─ Procesa hashtags
